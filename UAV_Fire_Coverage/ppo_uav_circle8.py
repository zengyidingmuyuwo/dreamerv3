"""
ppo_uav_circle8.py — PPO training for Circle 8 (fire coverage + obstacle avoidance).
...
SAMPLE_CENTER = os.path.join(_HERE, 'sample_data', 'circle_8_center.csv')
SAMPLE_POINTS = os.path.join(_HERE, 'sample_data', 'circle_8_points.csv')
...
def main():
    ...
    parser.add_argument('--elevation_tif', default=None)
    parser.add_argument('--episodes', type=int, default=MAX_EPISODES)
    parser.add_argument('--save_dir', default='./ppo_circle8_model')
    ...
    print(f'Circle 8: centre=({lat0:.4f},{lon0:.4f}), '
          f'radius={radius:.0f}m, fire_pts={len(fire_pts)}')

    if args.elevation_tif:
        obs_grid, cell_size = load_obstacle_map(
            args.elevation_tif, lat0, lon0, radius)
        print(f'Obstacle map loaded from GeoTIFF; '
              f'{obs_grid.sum()} obstacle cells')
    else:
        obs_grid, cell_size = make_synthetic_obstacle_map(radius)
        print('Using synthetic obstacle map (no GeoTIFF provided)')

    env = UAVFireObstacleEnv(fire_pts, radius, obs_grid, cell_size)
